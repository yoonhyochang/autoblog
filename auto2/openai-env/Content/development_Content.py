...
def save_to_file():
    ...
    if os.path.exists(Content_file_path):
        with open(Content_file_path, 'r', encoding='utf-8') as file:
            ...
            Content_data["development_Content"].append(content)  # 기존 내용에 새 내용을 추가
    else:
        Content_data = {"development_Content": [content]}  # 파일이 존재하지 않으면 새 데이터 구조를 생성
    ...
    messagebox.showinfo("Saved", "Your input was saved to development_Content.json.")  # 저장 완료 메시지를 표시

def run_script():
    def execute():
        script_path = "C:/Users/yhc93/OneDrive/바탕 화면/사업문서/마/auto2/openai-env/blogtextmaking/development_blogtextmaking.py"
        # subprocess.run을 사용하여 스크립트를 실행. 이제 스크립트 내에서 development_Content.json을 처리합니다.
