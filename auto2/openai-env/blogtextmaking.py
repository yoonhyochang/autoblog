...
# OpenAI 클라이언트 설정
client = OpenAI(api_key=os.environ.get("api 키"))
# GPT 모델을 사용해 요청 처리
...
for topic, category in zip(topics, categories):
    ...
    initial_completion = client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": f"마(yarm) 관련내용을 작성할건데 카테고리 :{category}와 주제 :{topic} 를 연관지어서 내용을 바탕으로 블로그 내용을 한글로 워드 프레스(SEO 형식)에다 작성 할 것이며, 최대한 길고 창의적으로 작성해주세요. 추가적인 설명은 넣지마."}]
    )
